...
(function (global, $) {
  ...
  var services = {
    ...
    createAnnotation: function (data) {
      data = data || {};
      data.annotationRange = JSON.stringify(data.annotationRange);
      return $.ajax({
        url:settings.baseUrl+'/api/annotation/note',
        type:'POST',
        dataType:'json',
        data:data
      })
    ...
    updateNote: function (data, id) {
      data = data || {};
      return $.ajax({
        url: settings.baseUrl+'/api/annotation/note/'+id,
        type: 'PUT',
        dataType: 'json',
        data: data
      })
  ...
  var PDFTask = global.Annotation = {
    ...
    initAnnotationCtrl: function () {
      ...
      $(document).on('mouseup', settings.pageAreaSelector ,  function (event) {
        ...
        if (settings.noteType == 'NOTE') {
          $(settings.annotationPopUp).removeClass('hidden');
          ...
          Save: function () {
            ...
            services.createAnnotation(settings.annotationData).done(function (data) {
              if (data.status == 'SUCCESS') {
                var last_note = data.last_note;
                var element_range = JSON.parse(last_note.annotationRange);
                var note_component = $(element_range.markElementName);
                note_component.attr('note-id', last_note.id);
      ...
      var notePopup = $(settings.updateNotePopup).dialog({
        ...
        buttons: {
          Update: function () {
            ...
            if(settings.annotationData.length){
              settings.annotationData[0].title = noteText;
              services.updateNote(settings.annotationData[0], settings.annotationData[0].annotation_id).done(function (data) {
                if (data.status == true) {
                  $(settings.updateNotePopup).dialog("close");
                  $(settings.updateNotePopup).addClass('hidden');
                }
              });
            }
      ...
      $('.select-note').unbind('click').on('click', function (e) {
        ...
        services.getAllNotes(data).done(function (response) {
          if (response.status == 'SUCCESS' && response.data.length) {
            ...
            $('#noteTextUpdate').html(note.title);
            settings.annotationData = response.data;
            $(settings.updateNotePopup).removeClass('hidden');
    ...
    renderAllNotes: function (annotationData) {
      ...
      annotationData.forEach(function (annotationDataItem) {
        ...
        contentText.html(annotation.title);
        annotation.aid = annotation.annotation_id;
        var annotationRange = annotation.annotationRange ? JSON.parse(annotation.annotationRange) : {};
        ...
        self.highlightSelectedText(selectionMetaData);
        var note_component = $(annotationRange.markElementName);
        note_component.attr('note-id', annotation.annotation_id);
        $(settings.sideBarTemplates.notes).append($el.html())
      });
